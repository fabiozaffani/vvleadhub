...
// Executar via `payload run src/seed.ts` (top-level await mantém o processo vivo).
...
const upsert = async (collection: CollSlug, slug: string, data: Record<string, unknown>) => {
  ...
  if (existing.docs.length > 0) {
    console.log(`= ${collection}/${slug} já existe — pulando`)
    return
  }
  await payload.create({ collection, data: { slug, ...data } })
  console.log(`+ ${collection}/${slug} criado`)
}
