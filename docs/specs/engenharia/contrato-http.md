...
Padrão de forma das respostas do `api-server`: envelope de erro tipado, paginação/filtro e idempotência. A forma é fixada no `openapi.yaml` (fonte de verdade → codegen p/ `contracts`), não em prosa — esta seção define o padrão que o spec implementa.
