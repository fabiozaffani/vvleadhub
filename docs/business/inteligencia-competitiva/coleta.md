...
**Cardinalidade:** Grupo (1) : Concorrente-Espaço (N) — o caso "grupo de um só espaço" é coberto naturalmente (todo Concorrente-Espaço pertence a um Grupo, mesmo unitário), então o modelo aceita tanto o **grupo consolidado** (vários Concorrente-Espaços) quanto o **espaço único** (grupo unitário).
...
### §1.2 — `Concorrente-Espaço` *(raiz observada)*
...
### §1.3 — `Canal`
**O que é:** a **procedência** — a superfície onde se observa um Concorrente-Espaço.
...
**Cardinalidade:** Concorrente-Espaço (1) : Canal (N). Toda Observação aponta para um Canal (a procedência) e para um Concorrente-Espaço (o dono).

### §1.4 — `Observação`
**O que é:** a **captura datada** de um estado ou fato de um Concorrente-Espaço, com Fonte e confiabilidade. É o átomo do radar — **uma** entidade, não cinco quase-sinônimas.
...
**Pertinência:** Observação → Concorrente-Espaço (N:1) e Observação → Canal (N:1).
...
### §1.5 — `Estética`
**O que é:** eixo de classificação de 1ª classe do Concorrente-Espaço (rústico, clássico, boho, garden, romântico…).
**Cardinalidade:** N:N — um Concorrente-Espaço pode ter mais de uma estética (e a mesma estética cobre vários rivais).

### §1.6 — `Disputa`
**O que é:** a relação associativa "este Concorrente-Espaço compete com tal **Espaço/categoria da VVF**". É o eixo central do radar — o cruzamento **estética × nível de mercado × Espaço-VVF disputado** é o mais usado.
...
- `INTEL-COL-02` (Política): a entidade-raiz observada é o **Concorrente-Espaço**; o **Grupo** é o agregador econômico. Cardinalidade Grupo (1) : Concorrente-Espaço (N), com o grupo de um só espaço coberto.
- `INTEL-COL-03` (Restrição): um Concorrente-Espaço pertence a **um** Grupo e pode **trocar de Grupo** numa aquisição **sem perder a identidade** longitudinal (re-parentamento); o Grupo de origem pode ir a "absorvido/arquivado".
- `INTEL-COL-04` (Restrição): toda Observação pertence a **um** Concorrente-Espaço e tem **um** Canal de procedência.
- `INTEL-COL-05` (Política): a **Estética** é um eixo de classificação (um Concorrente-Espaço pode ter mais de uma); a **classificação competitiva** tem dois eixos ortogonais — a **relação competitiva** {direto, indireto} (fechado) e o **nível de mercado** (a faixa; ilustrado por premium-full / premium-partial / mid / below, *extensível por dado*). *("Aspiracional" não é nível — convida ao me-too, atrita com `INTEL-GERAL-01`.)*
...
- `INTEL-COL-10` (Política): o **núcleo** de atributos do Concorrente-Espaço é identidade + as superfícies onde se observa (o que o robô precisa para apontar); o **perfil estratégico** (capacidade, pacote, fornecedores, anos, preço, SWOT) cresce com a curadoria e **não bloqueia** o núcleo.
- `INTEL-COL-11` (Restrição): cada Concorrente-Espaço associa-se a **≥1 Espaço/categoria da VVF** disputado (a Disputa) — o cruzamento estética × nível de mercado × Espaço-VVF é o eixo central do radar.
