...
- Build (D-16): o **Replit Agent** desenvolve o app inteiro (builder primário); **Cursor Composer** e **Claude Code** são auxiliares (auditoria, melhoria, debug, commits). Todos leem a mesma conduta via `AGENTS.md` (ponteiros em `CLAUDE.md`/`replit.md`/`.cursor/rules/*`). Conjunto de docs em `docs/` no repositório.
...
| D-16 | **Governança multi-agente (emenda à D-7):** **o Replit Agent é o builder primário — desenvolve o app inteiro** (site, admin, api-server, infra). **Cursor Composer** e **Claude Code** são **auxiliares/backup**: auditoria, revisão, melhorias, debug, commits, e trabalho onde tokens mais baratos/maior volume ajudam (Cursor é uso pontual do fundador; Claude Code roda os gates `/code-review`·`/audit-quality`·`/checklist-fase`). A fonte única de conduta deixa de ser o `CLAUDE.md` e passa a ser o **`AGENTS.md`** (tool-neutral; o Replit lê por `replit.md`, que se reescreve sozinho — pré-semeado para não se auto-governar; o Cursor lê `AGENTS.md`+`.cursor/rules`). `CLAUDE.md`, `replit.md` e `.cursor/rules/*` são **ponteiros** com invioláveis inline — em conflito, o `AGENTS.md` vence. Encontro em `packages/contracts` e `docs/` é **gated por CODEOWNERS** (aval do fundador). **Enforcement por máquina, não por prosa:** monorepo + CI em todo PR (typecheck/lint/dependency-cruiser/test/build/Lighthouse/axe) + `pnpm verify` como DoD — Fase 0a "guardrails como código", entregue **antes** das features (0b). **Sem branch protection mecânica** (decisão do fundador): branch+PR é convenção, check de CI vermelho é bloqueio. **Sync Replit→GitHub = modelo A (jun/2026):** o Replit commita em branch de trabalho (`replit/work`), **nunca na `main`**; "Publish" do Replit é só deploy, desacoplado do GitHub (publicar direto na main quebrou a CI e gerou duplicatas — proibido). Papéis ajustáveis em `docs/tasks/fase-0.md` | **fechada** |
