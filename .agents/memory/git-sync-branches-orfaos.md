...
**Gotcha — cópia derivada suja rebloqueia a poda (jun/2026):** a poda só roda com **working tree limpo** ("tree sujo não toca em branch"). Se o branch órfão "voltar" toda sessão **mesmo com o WO-CORE-024 ativo**, o culpado provável é um arquivo sujo perene — tipicamente **`.cursor/rules/00-canon-vvf.mdc`**, a cópia derivada do canon (regen por `sync-cursor-rules.sh` após bump do vvcore) que ficou **sem commit**. Toda sessão o `hook-session-sync.sh` vê a árvore suja, aborta antes de podar, e o órfão sobra. **Fix:** commitar a cópia derivada (caminho gated `.cursor/rules` → branch + PR, merge manual; ver PR #73), aí a árvore limpa e a poda volta a funcionar — manual agora, automática depois. O diagnóstico é `git status` no SessionStart: "pull pulou: alteracoes locais" = árvore suja = poda não rodou. Liga-se ao gotcha de canon do ARQUITETURA-IA §6.6 (regen de cópia derivada roda só após o commit do canon).
