...
**Bridge pro Claude Code:** o `setup-links.sh` do vvcore (função `link_repo_skills`, disparada pelo hook `SessionStart`) cria o link por máquina **`.claude/skills → .agents/skills`** — junction no Windows, symlink no macOS/Linux, gitignored, recriado a cada sessão. O Claude varre `.claude/skills` (único lugar que ele lê), mas os arquivos reais vivem aqui.

- **Uma skill = uma subpasta com `SKILL.md`** (`<nome>/SKILL.md`); o nome do diretório é o que se invoca.
- **Opt-in:** esta pasta existir é o gatilho do link. Vazia (só com este README), fica **pré-cablada** — o link já existe, pronto pra primeira skill.
