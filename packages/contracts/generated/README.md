...
- **`payload generate:types`** (admin) → tipos das collections do Payload, consumidos pelo `site/` para tipar as respostas da API de conteúdo. **Não** escrever esses tipos à mão no site (ver `AGENTS.md`). O CI falha se o gerado divergir do commitado.
