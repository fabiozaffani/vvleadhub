...
- **CODEOWNERS (caminhos gated):** `packages/contracts`, `docs/`, `AGENTS.md`/`CLAUDE.md`, `.claude/`, `.cursor/rules`, `.github/`, `infra/` — aval do fundador, nunca auto-mergeado. (`.claude/` gated por D-1 — é o gate de merge do `settings.json` editado pelo agente.)
